...
public class java1 {
	...
	public static void main(String[] args) {
		...
		for(int i = 0; i < bingo.length; i++) {
			...
			for(int k = 0; k < i; k++) {
				if(bingo[k] == bingo[i]) {
					i--; break; // 이전 위치의 값이 중복인지 확인하기 위해서 사용, 끝이면 break문으로 끝
				}
			}
		}
		...
		// 배열에 저장된 데이터 중에서 5의 배수는 몇 개인지 출력하기
		
//		int[] number = new int[20];
//		
//		for(int i = 0; i < number.length; i++) {
//			number[i] = (int)Math.floor(Math.random() * 30)+1;
//		}
//		
...
//		System.out.println("5의 배수 몇개? " + count);
		...
		// 배열 생성방법, 배열에 데이터 저장하는 방법, 배열에 저장된 데이터 추출 또는 출력
