...
<!-- 
	...
	JSP 페이지를 생성하고 서버의 의해서 페이지가 동작할 때 서버에서 페이지에게 객체를 할당해준다.
	
	내장객체는 9가지이다.
		서버 - application : 처음 서버가 시작할 때 생성되는 객체
		연결 - session : 사용자가 서버에 처음 요청할 때 생성되고 사용자가 브라우저를 종료하기 전까지
			살아있는 객체
		페이지 - request : 사용자의 요청 내용을 저장하는 객체
			   response : 요청에 대한 응답 정보를 저장하는 객체
			   out : 출력 객체
			   page : JSP 페이지 자체를 나타낸다.
			   config : JSP 페이지의 설정 정보를 저장해둔 객체
			   exception : JSP 페이지의 예외 발생을 처리하는 개체
...
<body>
	...
	%>
	<!--  
	input 태그에 입려한 내용에 대한 처리를 requestTest.jsp 주소로 요청
	 -->
